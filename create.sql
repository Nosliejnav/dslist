    ...
    create table dslist.tb_belonging (
        position integer,
    ...
    create table dslist.tb_game_list (
        id bigint generated by default as identity,
        name varchar(255),
        primary key (id)
    );

    alter table if exists dslist.tb_belonging 
       add constraint FKrchwdikeu66uky1hf75ym1kh 
       foreign key (list_id) 
       references dslist.tb_game_list;

    alter table if exists dslist.tb_belonging 
       add constraint FK2slybclee7wdfxhfltbvqkgpg 
       foreign key (game_id) 
       references dslist.tb_game;
